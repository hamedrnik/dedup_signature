...
impl TextProfileSignature {
  pub fn default() -> Self {
    TextProfileSignature {min_token_length: 2, quant_rate: 0.01} 
  }
  ...
  pub fn generate_sign(&self, text: &str) -> String {
        ...
        if current_token.chars().count() > 0 {
          if current_token.chars().count() > self.min_token_length {
            if !tokens.contains_key(&current_token) {
              let tok = Token { count: 0, term: current_token.clone()};
    ...
    let mut quantized_tokens: Vec<Token> = tokens.iter().fold(Vec::new(), |mut memo, (_, val)| {
    ...
    let quantized_frequency_vec: Vec<String> = quantized_tokens.iter().map(|a| {
      format!("{} {}", a.term, a.count)
    }).collect();
